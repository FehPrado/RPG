class Heroi
    ...
    def initialize(nome, classe, force, resistencia, vitalidade, frase)
        ...
        @vitalidade = vitalidade + calsse.vitalidade
        @ataque = @force + @pode_arma
        @defesa = @resistencia + @defesa_armadura
        @hp = classe.hp * @vitalidade
        @vigor = classe.vigor * (@resistencia + @force) /4
        @frase = frase
...
class Inimigo
    ...
    def apareceu
        ...
        puts "\e[31m- Arararaaa eu, #{self.nome} com meus #{self.ataque} grandiosos pontos de ataque e minha indestrutivel defesa de #{self.defesa} e tenho #{self.hp} estou aqui para acabar com você e assim finalmente consegui #{self.objetivo}\e[0m
      "
    end
end
